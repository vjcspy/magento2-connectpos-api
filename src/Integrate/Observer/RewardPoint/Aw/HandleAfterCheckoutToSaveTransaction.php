...
class HandleAfterCheckoutToSaveTransaction implements \Magento\Framework\Event\ObserverInterface {
    ...
    public function execute(Observer $observer) {
        ...
        if ($order->getData('retail_id') && $order->getData('aw_reward_points')) {
            ...
            $giftCardPaymentId = $this->paymentHelper->getPaymentIdByType(\SM\Payment\Model\RetailPayment::REWARD_POINT_PAYMENT_TYPE);
            if ($currentShift->getData('id') && $giftCardPaymentId) {
                ...
                $transaction->setData('payment_id', $giftCardPaymentId)
                            ...
                            ->setData('payment_type', \SM\Payment\Model\RetailPayment::REWARD_POINT_PAYMENT_TYPE)
